...
// ✅ Importar modelos desde el índice (con asociaciones)
const {
  Venta,
  VentaDetalle,
  Producto,
...
// --- Helpers ---
/** Busca el cliente "Consumidor Final". */
...
const crearVenta = async (req, res) => {
  const {
    detalles, // flujo MOSTRADOR
    pedido_id = null, // NUEVO: flujo MESA/DELIVERY/TERRAZA
  ...
  try {
    // 1) Cliente (Consumidor Final por defecto)
    let clienteParaVentaId = clienteIdRecibido;
    if (!clienteParaVentaId) {
      const cf = await findConsumidorFinal(t);
      if (!cf) throw new Error('Cliente "Consumidor Final" no encontrado. Asegúrese de que exista.');
      clienteParaVentaId = cf.id;
    }

    // 2) Numeración de factura (lock)
    ...
    if (pedido_id) {
      // Tomar desde pedido_detalles y agrupar por producto_id
      ...
      // Traer costo_envio y mesa para cierre
      ...
      // 🔴 Cambiado: liberar mesa para 'mesa' o 'terraza'
      mesaId = ['mesa', 'terraza'].includes(ped.tipo) ? ped.mesa_id : null;
    } else {
      // Flujo MOSTRADOR
...
// ========================== LISTAR VENTAS (con tipo terraza) ==========================
const listarVentas = async (req, res) => {
  const { desde, hasta, cliente_id, estado, includeDetails, includeClient, q, tipo } = req.query;

  // Paginación opcional
  const page = Number(req.query.page ?? 0); // 0 => sin paginación (compat)
  ...
  // búsqueda q (cliente / campos delivery)
  if (q && String(q).trim() !== '') {
  ...
  try {
    ...
    return res.json({ data: rows, page: Math.max(page, 1), pageSize: limit, total: count, totalPages });
  } catch (error) {
...
// (resto del archivo original sin cambios materiales)
...
const resumenVentas = async (req, res) => {
  ...
  try {
    ...
    const totales = await Venta.findAll({
      where: whereClause,
      attributes: [ [fn('COUNT', col('id')), 'cantidad_ventas'], [fn('SUM', col('total')), 'total_general'] ],
      raw: true
    });
    ...
    const porTipoSql = `
      SELECT COALESCE(p.tipo, 'mostrador') AS tipo, SUM(v.total)::bigint AS monto
      FROM ventas v
    ...
    return res.json({ rango: { desde, hasta }, total_general, cantidad_ventas, ticket_promedio, por_metodo, por_tipo });
  } catch (error) {
...
const generarFacturaPDF = async (req, res) => {
  try {
    ...
    const configEmpresa = await ConfiguracionEmpresa.findByPk(1);
    if (!configEmpresa) { return res.status(500).json({ error: 'Configuración de la empresa no encontrada.' }); }
...
const anularVenta = async (req, res) => {
  ...
  try {
    const venta = await Venta.findByPk(id, {
      include: [ { model: VentaDetalle, as: 'detallesDeVenta' }, { model: Cliente, as: 'cliente' } ],
      transaction: t
    });
    ...
    switch (venta.metodo_pago) {
      case 'efectivo':
        await MovimientoCaja.create({
          tipo: 'egreso', monto: venta.total,
          descripcion: `Reembolso por anulación de Factura #${venta.numero_factura ?? 'N/A'} (Efectivo)`,
          fecha: new Date(), venta_id: venta.id, usuario_id: req.usuario ? req.usuario.id : null
        }, { transaction: t });
        break;
      case 'tarjeta':
      case 'transferencia':
...
const generarArqueo = async (req, res) => {
  ...
  try {
    ...
    if (!consumidorFinalCliente) { return res.status(404).json({ message: 'Cliente "Consumidor Final" no configurado.' }); }
    ...
    const whereClause = {
      ...
      id: { [Op.notIn]: literal(`(SELECT venta_id FROM cierre_global_detalles WHERE cierre_global_id IS NOT NULL)`) }
    };
    ...
    totalesPorMetodo.forEach((item) => {
      ...
      if (resumenTotales[metodo] !== undefined) { resumenTotales[metodo] = total; }
      totalGeneral += total;
    });
    ...
    return res.status(500).json({ message: 'Error interno del servidor al generar el arqueo.', details: error.message });
  }
};
...
const registrarCierreGlobal = async (req, res) => {
  ...
  try {
    ...
    if (ventas_ids_incluidas.length > 0) {
      const detalleValues = ventas_ids_incluidas.map((ventaId) => ({ cierre_global_id: cierreGlobal.id, venta_id: ventaId }));
      await CierreGlobalDetalle.bulkCreate(detalleValues, { transaction: t });
    }

    await t.commit();
    return res.status(201).json({ message: 'Cierre global registrado exitosamente.', cierreGlobalId: cierreGlobal.id });
  } catch (error) {
    await t.rollback();
    console.error('Error al registrar el cierre global:', error);
    return res.status(500).json({ message: 'Error interno al registrar el cierre global.', error: error.message });
  }
};
...
module.exports = {
  ...
  listarCierresGlobales: async (req, res) => {
    ...
    try {
      const cierres = await CierreGlobal.findAll({ where, order: [['fecha_cierre', 'DESC']], include: includeOptions });
