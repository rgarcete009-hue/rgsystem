...
// Valida que la mesa pertenezca al sector esperado
async function assertMesaSector(mesa_id, esperado, t) {
  const mesa = await Mesa.findByPk(mesa_id, { transaction: t, lock: t.LOCK.UPDATE });
  if (!mesa) throw new Error('Mesa no encontrada.');
  const sec = (mesa.sector || '').toLowerCase();
  if (esperado === 'terraza' && sec !== 'terraza') {
    throw new Error('La mesa seleccionada no pertenece a la Terraza.');
  }
  if (esperado === 'salon' && sec === 'terraza') {
    throw new Error('La mesa seleccionada pertenece a la Terraza (use tipo "terraza").');
  }
  return mesa;
}

/**
 * Abre un pedido. Si es mesa/terraza y ya hay uno abierto para esa mesa y tipo, reutiliza ese pedido.
 * Body esperado:
 * {
 *  tipo: 'mesa' | 'delivery' | 'terraza',
 *  mesa_id?: UUID,
 *  cliente_id?: UUID,
 *  delivery?: {
 *    telefono?: string,
...
const abrirPedido = async (req, res) => {
  ...
  try {
    ...
    if (!['mesa', 'delivery', 'terraza'].includes(tipo)) {
      await t.rollback();
      return res.status(400).json({ message: 'Tipo inválido. Use mesa, delivery o terraza.' });
    }

    // Si es mesa/terraza, validar sector y reusar pedido abierto
    if (['mesa','terraza'].includes(tipo) && mesa_id) {
      await assertMesaSector(mesa_id, tipo === 'terraza' ? 'terraza' : 'salon', t);
      const existente = await Pedido.findOne({
        where: { tipo, mesa_id, estado: 'abierto' },
        transaction: t
      });
      if (existente) {
        await t.commit();
    ...
    const p = await Pedido.create({
      tipo,
      mesa_id: ['mesa','terraza'].includes(tipo) ? mesa_id : null,
...
const agregarDetalles = async (req, res) => {
  try {
    ...
    const rows = items.map((i) => {
      ...
      return {
        pedido_id: id,
        producto_id: Number(i.producto_id), // INTEGER
    ...
    for (const r of rows) {
      if (!r.producto_id || r.cantidad <= 0 || r.precio_unitario < 0) {
        return res.status(400).json({ message: 'Ítem inválido (producto_id, cantidad > 0 y precio_unitario >= 0 son obligatorios).' });
      }
    }
...
/**
 * Lista pedidos filtrando por tipo y estado (para "Delivery activos", etc.)
 */
const listarPedidos = async (req, res) => {
  try {
    ...
    const pedidos = await Pedido.findAll({
      ...
      attributes: [
        'id','tipo','estado','created_at','costo_envio',
        'delivery_telefono','delivery_direccion','delivery_referencia','delivery_nombre',
        'cliente_id','mesa_id'
      ],
      include: [
        { model: Cliente, as: 'cliente', attributes: ['id','nombre','ruc'], required: false }
      ]
    });
...
/**
 * Actualiza el estado del pedido. Si es mesa/terraza y finaliza, libera mesa.
 */
const actualizarEstadoPedido = async (req, res) => {
  ...
  try {
    ...
    if (['mesa','terraza'].includes(p.tipo) && p.mesa_id && (estado === 'cobrado' || estado === 'cancelado')) {
      await Mesa.update({ estado: 'libre' }, { where: { id: p.mesa_id }, transaction: t });
    }
